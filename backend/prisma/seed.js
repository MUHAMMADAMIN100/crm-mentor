...
// Each student is paired 1-to-1 with the teacher at the same index.
const STUDENTS = [
...
// One course per teacher (10 courses total).
const COURSES = [
...
async function main() {
  ...
  // ===== STUDENTS (paired 1-to-1 with teachers by index) =====
  ...
  for (let i = 0; i < STUDENTS.length; i++) {
    ...
    // If user existed but profile is missing or detached from teacher — fix it.
    if (!u.studentProfile) {
  ...
  for (let i = 0; i < COURSES.length; i++) {
    ...
    // Grant student access to teacher's course (idempotent via @@unique)
  ...
  // ===== PRIVATE CHATS between each (teacher, student) pair =====
  // So students appear instantly in teacher's chat list and vice-versa.
