...
export class AiService {
  ...
  suggestions(role: 'ADMIN' | 'TEACHER' | 'STUDENT'): string[] {
    if (role === 'ADMIN') return [
      'Кто скоро отвалится? (риск оттока)',
      'Какие подписки заканчиваются на этой неделе?',
      'Какие учителя без активности 7+ дней?',
      'Какие учителя без учеников или курсов?',
      'Кто из менеджеров работает лучше всех?',
      'Краткая сводка платформы и рекомендации',
      'Покажи общую выручку и MRR',
    ];
